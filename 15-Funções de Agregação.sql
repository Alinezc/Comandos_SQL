...
--Exemplo com Coluna sem Função de Agregação e sem Group By (Erro)
 ...
 MAX(UnitPriceDiscount) AS DEsconto_Máximo, ProductID
 FROM Sales.SalesOrderDetail;
...
--Função de Agregação em Conjunto com Outras Funções
 ...
 --DISTINCT com Função de Agregação
 ...
 SELECT AVG(Weight) AS "Peso_Médio_dos_Produtos(Usendo AVE - OK)",
 SUM(Weight) / COUNT(Weight) AS [Peso_Mádio_dos_Produtos(Sem AVG - ON)],
